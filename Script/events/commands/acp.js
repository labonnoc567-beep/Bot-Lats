module.exports.config = {
  ...
  description: "Accept or delete friend requests via FB UID",
...
module.exports.handleReply = async ({ handleReply, event, api }) => {
  ...
  if (args[0] == "add") {
    ...
    return api.sendMessage("⚠️ Please use either `add` or `del` followed by number(s) or `all`", event.threadID, event.messageID);
  }
  ...
  for (const stt of targetIDs) {
    ...
    if (!u) {
      failed.push(`❌ Invalid number: ${stt}`);
      continue;
    }
  ...
  const action = args[0] == "add" ? "added to" : "deleted from";
  const emoji = args[0] == "add" ? "✅" : "🗑️";
  const message = success.map(name => `${emoji} Successfully ${action} my friend list: "${name}"`).join("\n");

  return api.sendMessage(
    `${message}${failed.length > 0 ? `\n❌ Failed with: ${failed.join(", ")}` : ""}`,
    event.threadID,
    event.messageID
  );
};
